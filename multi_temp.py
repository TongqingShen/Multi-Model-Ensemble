...
# import data
# base period
obs = pd.read_excel(io=r'F:\Research\SCI\res\his\base_obs_tas.xlsx',header=None)
train = pd.read_excel(io=r'F:\Research\SCI\res\his\train_mods_corr_tas.xlsx',header=0)
valid = pd.read_excel(io=r'F:\Research\SCI\res\his\valid_mods_corr_tas.xlsx',header=0)
data_obs = np.array(obs)
data_train = np.array(train)
data_valid = np.array(valid)
y_train = data_obs[:372]
# future data
ssp126 = pd.read_excel(io=r'F:\Research\SCI\res\future\ssp_126_corr_tas.xlsx',header=0)
ssp245 = pd.read_excel(io=r'F:\Research\SCI\res\future\ssp_245_corr_tas.xlsx',header=0) 
ssp585 = pd.read_excel(io=r'F:\Research\SCI\res\future\ssp_585_corr_tas.xlsx',header=0)
...
param_BP = {
    'hidden_layer_sizes': Integer(low=50, high=150, name='hidden_layer_sizes'),
...
params_pr_BP = search_BP.best_params_
#%% LSTM
# Create LSTM model
...
def create_lstm_model(hidden_units=64, epochs=10, batch_size=32, dropout=0.2, l2_lambda=0.01):  
    model = Sequential()  
...
keras_regressor = KerasRegressor(build_fn=create_lstm_model, epochs=10, verbose=1)  
...
base_LSTM = denormalize_data(LSTM_fit, np.min(y_train),np.max(y_train))
# future prediction
